...
class ClusterSettingsArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[bool] action_destructive_requires_name: When set to true, you must specify the index name to delete an index and it is not possible to delete all indices with
        ...
        :param pulumi.Input[int] cluster_max_shards_per_node: The total number of primary and replica shards for the cluster, this number is multiplied by the number of non-frozen
               data nodes; shards for closed indices do not count toward this limit
        :param pulumi.Input[int] cluster_max_shards_per_node_frozen: The total number of primary and replica frozen shards, for the cluster; Ssards for closed indices do not count toward
        ...
        :param pulumi.Input[str] cluster_persistent_tasks_allocation_recheck_interval: A time string controling how often assignment checks are performed to react to whether persistent tasks can be assigned
               to nodes
        :param pulumi.Input[str] cluster_routing_allocation_allow_rebalance: Specify when shard rebalancing is allowed (always, indices_primaries_active, indices_all_active)
        ...
        :param pulumi.Input[float] cluster_routing_allocation_balance_index: Weight factor for the number of shards per index allocated on a node, increasing this raises the tendency to equalize
               the number of shards per index across all nodes
        :param pulumi.Input[float] cluster_routing_allocation_balance_shard: Weight factor for the total number of shards allocated on a node, increasing this raises the tendency to equalize the
               number of shards across all nodes
        :param pulumi.Input[float] cluster_routing_allocation_balance_threshold: Minimal optimization value of operations that should be performed, raising this will cause the cluster to be less
        ...
        :param pulumi.Input[bool] cluster_routing_allocation_disk_include_relocations: Whether the allocator will take into account shards that are currently being relocated to the target node when computing
               a node’s disk usage
        ...
        :param pulumi.Input[int] cluster_routing_allocation_node_concurrent_incoming_recoveries: How many incoming recoveries where the target shard (likely the replica unless a shard is relocating) are allocated on
               the node
        :param pulumi.Input[int] cluster_routing_allocation_node_concurrent_outgoing_recoveries: How many outgoing recoveries where the source shard (likely the primary unless a shard is relocating) are allocated on
               the node
        ...
        :param pulumi.Input[bool] cluster_routing_allocation_same_shard_host: Perform a check to prevent allocation of multiple instances of the same shard on a single host, if multiple nodes are
               started on the host
        ...
        :param pulumi.Input[str] indices_breaker_fielddata_limit: The percentage of memory above which if loading a field into the field data cache would cause the cache to exceed this
        ...
        :param pulumi.Input[str] indices_breaker_request_limit: The percentabge of memory above which per-request data structures (e.g. calculating aggregations) are prevented from
               exceeding
        ...
        :param pulumi.Input[str] script_max_compilations_rate: Limit for the number of unique dynamic scripts within a certain interval that are allowed to be compiled, expressed as
